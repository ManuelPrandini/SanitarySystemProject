...
create table `building` (
   `hospital_department_id_departm`  integer not null,
   `id_building`  integer not null,
  primary key (`hospital_department_id_departm`, `id_building`)
);
...
alter table `building`   add index fk_building_building (`id_building`), add constraint fk_building_building foreign key (`id_building`) references `building` (`id_building`);
