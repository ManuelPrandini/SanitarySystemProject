...
-- Check_Visit [ent29]
create table `visit` (
   `visit_oid`  integer  not null,
  primary key (`visit_oid`)
);
...
alter table `visit`  add column  `calendar_oid`  integer;
alter table `visit`   add index fk_visit_calendar (`calendar_oid`), add constraint fk_visit_calendar foreign key (`calendar_oid`) references `calendar` (`oid`);
...
alter table `drug_prescription`   add index fk_drug_prescription_visit (`visit_oid`), add constraint fk_drug_prescription_visit foreign key (`visit_oid`) references `visit` (`visit_oid`);
...
-- GEN FK: Check_Visit --> Action
alter table `visit`   add index fk_visit_visit (`visit_oid`), add constraint fk_visit_visit foreign key (`visit_oid`) references `visit` (`oid`);


-- GEN FK: Clinical_Surgery --> Action
alter table `clinical_surgery`   add index fk_clinical_surgery_visit (`visit_oid`), add constraint fk_clinical_surgery_visit foreign key (`visit_oid`) references `visit` (`oid`);


-- GEN FK: Recovery --> Action
alter table `recovery`   add index fk_recovery_visit (`visit_oid`), add constraint fk_recovery_visit foreign key (`visit_oid`) references `visit` (`oid`);
