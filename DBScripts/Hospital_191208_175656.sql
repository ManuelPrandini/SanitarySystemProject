...
create table `without_prescription` (
   `sell_oid`  integer  not null,
  primary key (`sell_oid`)
);
...
-- CodaprontosoccorsoEmergencyOperation [rel27]
alter table `emergency_operation`  add column  `codaprontosoccorso_oid`  integer;
alter table `emergency_operation`   add index fk_emergency_operation_codapro (`codaprontosoccorso_oid`), add constraint fk_emergency_operation_codapro foreign key (`codaprontosoccorso_oid`) references `codaprontosoccorso` (`oid`);
...
alter table `sell`  add column  `pharmacist_pharmacist_code`  varchar(255);
alter table `sell`   add index fk_sell_pharmacist (`pharmacist_pharmacist_code`), add constraint fk_sell_pharmacist foreign key (`pharmacist_pharmacist_code`) references `pharmacist` (`pharmacist_code`);
...
alter table `emergency_operation`  add column  `emergency_room_id_room`  integer;
alter table `emergency_operation`   add index fk_emergency_operation_emerg_2 (`emergency_room_id_room`), add constraint fk_emergency_operation_emerg_2 foreign key (`emergency_room_id_room`) references `emergency_room` (`room_id_room`);
...
-- Hospital_Codaprontosoccorso [rel41]
...
alter table `without_prescription`   add index fk_without_prescription_sell (`sell_oid`), add constraint fk_without_prescription_sell foreign key (`sell_oid`) references `sell` (`oid`);


-- GEN FK: Emergency_Room --> Room
alter table `emergency_room`   add index fk_emergency_room_room (`room_id_room`), add constraint fk_emergency_room_room foreign key (`room_id_room`) references `room` (`id_room`);
...
alter table `with_prescription`   add index fk_with_prescription_sell (`sell_oid`), add constraint fk_with_prescription_sell foreign key (`sell_oid`) references `sell` (`oid`);
