...
create index `idx_user_group_user` on `user_group`(`user_cf`);
...
create index `idx_hospital_department_hospit` on `hospital_department`(`hospital_id_hospital`);
...
create index `idx_room_building` on `room`(`building_id_building`);
...
create index `idx_action_patient` on `action`(`patient_cf`);
...
create index `idx_doctor_hospital_doctor` on `doctor_hospital`(`doctor_cf`);
create index `idx_doctor_hospital_hospital` on `doctor_hospital`(`hospital_id_hospital`);
...
create index `idx_pharmacy_city` on `pharmacy`(`city_oid`);
...
create index `idx_pharmacist_pharmacy` on `pharmacist`(`pharmacy_oid`);
...
create index `idx_visit_visit_room` on `visit`(`visit_room_id_room`);
...
create index `idx_quantity_drug_pharmacy` on `quantity_drug`(`pharmacy_oid`);
...
create index `idx_clinical_surgery_surgery_r` on `clinical_surgery`(`surgery_room_id_room`);
...
create index `idx_without_prescription_patie` on `without_prescription`(`patient_cf`);
...
create index `idx_hospital_city` on `hospital`(`city_oid`);
...
create index `idx_recovery_bed_place` on `recovery`(`bed_place_oid`);
...
create index `idx_action_calendar` on `action`(`calendar_oid`);
...
create index `idx_quantity_drug_drug` on `quantity_drug`(`drug_id`);
...
create index `idx_drug_description_drug` on `drug_description`(`drug_id`);
...
create index `idx_without_prescription_drug` on `without_prescription`(`drug_id`);
...
create index `idx_emergency_room_hospital` on `emergency_room`(`hospital_id_hospital`);
...
create index `idx_visit_calendar_visit_room` on `visit_calendar`(`visit_room_id_room`);
...
create index `idx_surgery_calendar_surgery_r` on `surgery_calendar`(`surgery_room_id_room`);
...
create index `idx_recovery_calendar_bed_plac` on `recovery_calendar`(`bed_place_oid`);
...
create index `idx_action_doctor` on `action`(`doctor_cf`);
...
create index `idx_drug_prescription_visit` on `drug_prescription`(`visit_oid`);
...
create index `idx_drug_prescription_drug` on `drug_prescription`(`drug_id`);
...
create index `idx_assignement_pharmacist` on `assignement`(`pharmacist_cf`);
...
create index `idx_with_prescription_drug_pre` on `with_prescription`(`drug_prescription_oid`);
...
create index `idx_emergency_operation_emerge` on `emergency_operation`(`emergency_code_code`);
...
create index `idx_emergency_operation_emer_2` on `emergency_operation`(`emergency_room_oid`);
...
create index `idx_doctor_medical_specializat` on `doctor`(`medical_specialization_namespe`);
...
create index `idx_building_hospital_departme` on `building`(`hospital_department_id_departm`);
...
create index `idx_city_country` on `city`(`country_oid`);
...
create index `idx_user_city` on `user`(`city_oid`);
...
create index `idx_bed_place_recovery_room` on `bed_place`(`recovery_room_id_room`);
...
create index `idx_user_city_2` on `user`(`city_oid_2`);
